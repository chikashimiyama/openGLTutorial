...
void GLComponent::initialise()
{
    ...
    if (!(shader_->addVertexShader (BinaryData::varying_vert)
          && shader_->addFragmentShader (BinaryData::varying_frag)
          && shader_->link()))
    {
    ...
    auto viewMatrix = glm::lookAt(glm::vec3(0.0f, 0.f, 1.f), glm::vec3(0.0f, 0.f, 0.f), glm::vec3(0.f, 1.f, 0.f));
...
void GLComponent::render()
{
    ...
    shader_->use();
    auto modelMatrix = glm::rotate(glm::mat4(1.0f), glm::radians(degree_), glm::vec3(0.f, 0.f, 1.f));
